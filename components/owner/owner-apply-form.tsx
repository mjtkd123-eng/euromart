...
export function OwnerApplyForm({
  onSuccess,
  submitLabel = "입점 신청 제출",
}: {
  onSuccess?: () => void
  submitLabel?: string
}) {
  ...
  return (
    <form onSubmit={(e) => void onSubmit(e)} className="flex flex-col gap-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="대표자 이름" id="fullName">
          <Input id="fullName" required value={form.fullName} onChange={(e) => set("fullName", e.target.value)} className="h-11" />
        </Field>
        <Field label="대표자 연락처" id="phone">
          <Input id="phone" required value={form.phone} onChange={(e) => set("phone", e.target.value)} className="h-11" placeholder="+43 …" />
        </Field>
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="사업자명 / 상호" id="legalName">
